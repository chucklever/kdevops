...
WORKFLOWDIR=${TOPDIR}/workflows/rdstcp
...
{
	while true; do
		echo "== kernel-ci rdstcp test loop $COUNT start: $(date)" > $KERNEL_CI_FAIL_LOG
		echo "/usr/bin/time -f %E make rdstcp-baseline" >> $KERNEL_CI_FAIL_LOG
		/usr/bin/time -p -o $KERNEL_CI_LOGTIME make rdstcp-baseline >> $KERNEL_CI_FAIL_LOG
		ANSIBLE_CALL_RET=$?
